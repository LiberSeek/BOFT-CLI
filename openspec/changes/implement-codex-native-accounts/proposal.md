...
- 原子明文 Vault 保存 metadata 和全部凭据副本，当前身份由官方凭据推导；启动／列表刷新同步外部登录与退出，统一事务处理显式切换、首次激活、重登、退出和恢复。
- 设置页添加使用私有认证 staging，与任务后台不并行；已有身份时只保存新账号。Desktop 原生登录／取消／退出直接交给官方，不进入 Host 事务；Host 跟随官方凭据事实。
