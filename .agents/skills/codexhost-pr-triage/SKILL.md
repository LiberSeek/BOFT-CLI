...
|---|---|---|---|---|
| #123 标题 | SIMPLIFY | 功能补齐真实缺口，但新增缓存重复已有状态 | 进行中 / 无 | 复用既有状态，删除第二套缓存 |

固定看板包含四列建议、表格视图、搜索/排序、可展开详情和 JSON 导出。卡片显示价值、下一步与 HEAD；CI/冲突独立展示。部分结果与跳过项不会被隐藏成“已完成全部队列”。只有打开 PR、查看证据等只读链接；需要深审时在 `action` 中给出对该编号运行 `/review` 的提示。
