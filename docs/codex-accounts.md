...
- 刷新额度查询已登录的 ChatGPT 账号，以及带本地 `base_url` 与 API Key 的 API 账号。ChatGPT 账号展示官方窗口百分比；API 账号请求 Provider 的 `GET /v1/usage`，按返回的 `remaining` / `unit`（或 `quota.remaining`、`balance`）展示余额，而不是合成百分比。加载、读取失败、暂无数据分别展示；失败可重试，未知数据不按 0% 处理。单个账号的请求不会阻塞其他账号的额度展示，页面关闭后的响应不会更新页面。
