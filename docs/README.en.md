...
>
> For CodeBuddy, — means the capability is not integrated into the Host, not necessarily absent from the CLI. Image input and a dedicated Teams UI are also not integrated. See [CodeBuddy integration](codebuddy-harness-integration.md) for versions, remote execution requirements and verification scope.
