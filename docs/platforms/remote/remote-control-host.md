...
- Both computers run the same BOFT CLI version and launch Codex Desktop through BOFT.
...
First, make sure regular Codex tasks work over Remote Control. Pairing failures, missing environments, and account authorization errors come from Remote Control itself, not BOFT.

- **`unknown variant codexhost/harness/inspect`**: upgrade and restart BOFT on both computers, then reconnect the environment.
- **Bridge fails to start, or `no active process for process handle`**: make sure Codex Desktop on Windows was launched through BOFT, restart it, and reconnect the environment.
...
- **`Claude inbound is disabled`**: Claude Code integration is turned off in BOFT on the Windows machine. Turn it on and try again.
