...
Codex Desktop's official Remote Control can expose a Windows workspace to another paired Codex Desktop without SSH. When both Desktops are launched through the same BOFT CLI build, Harnesses installed and authenticated on the controlled Windows machine can be selected from the controlling Desktop.

This integration is experimental. It preserves OpenAI's pairing, account authentication, relay, environment discovery, and stock app-server protocol. BOFT CLI does not operate a replacement relay or open a new TCP listener.

## Requirements

- The controlled Host runs Windows. A macOS controller is currently the verified configuration.
- Install the same BOFT CLI build on both computers and launch both Codex Desktop instances through boft.
- Sign in to the ChatGPT account required by Codex Remote Control on both computers. Complete official Remote Control pairing first; BOFT CLI cannot bypass account or device authorization.
...
The official relay accepts only stock app-server methods and rejects private methods such as `codexhost/harness/inspect`. BOFT CLI therefore carries its private LF-delimited app-server stream inside the stock `process/spawn`, `process/writeStdin`, `process/kill`, `process/outputDelta`, and `process/exited` methods:
...
- Bridge initialization timeout after restarting Windows: retry the failed action. BOFT CLI abandons a bridge that does not complete app-server initialization within 15 seconds and the next request starts a fresh bridge against the current runtime descriptor.
- The Harness is absent but stock Codex works: run BOFT CLI connection diagnostics on the controller, then verify the Harness installation and login on Windows.
- `Claude inbound is disabled`: the request reached the controlled Windows BOFT CLI successfully, but Claude Code integration is disabled there. Enable Claude in the Windows BOFT CLI GUI/config before retrying.
- Pairing fails before an environment appears: complete official Remote Control authorization with the required ChatGPT account; this is outside the BOFT CLI bridge.
