...
Codex identity and quota readings are scoped to their Host; local Accounts do not appear in a remote Composer. Composer has one Codex option and no per-draft Account override. After switching Hosts or replacing the request client, old responses cannot overwrite the current Composer. SSH retains remote native single-account authentication, without forwarding local credentials or opening a remote managed Account vault. Existing Threads retain their Harness ownership, not a per-Thread Account binding. In locally managed mode, Account switching is global and affects subsequent Turns of existing Codex Threads; see [Account settings](codex-accounts.md).

Settings → Session Import is scoped to that same current Host. With a remote Composer selected, it lists and maps only Sessions stored on the SSH host, and opens the imported Thread through that remote Host. Claude discovery runs directly on Linux SSH hosts and through the Aqua broker on managed macOS hosts; only bounded Session metadata and the validated native identity cross the broker, never Transcript content or credentials. Close the Session in its native Claude client before importing when activity is reported as unknown. Install the same codexhost version on both machines and reconnect after an upgrade so the remote runtime and broker expose the matching import methods.
