...
describe("Codex Account auth projection", () => {
  it("marks vault Accounts as ChatGPT when the official home is not API", () => {
    expect(projectCodexAccountAuth(snapshot, { kind: "chatgpt", identity: "" })).toMatchObject({
      currentAccountId: "account-a",
      accounts: [
        {
          accountId: "account-a",
          authKind: "chatgpt",
          authIdentity: "a@example.com",
        },
      ],
    });
  });
