...
function withChatgptAuth(account: CodexAccountSummary): CodexAccountSummary {
  return {
    ...account,
    authKind: account.authKind ?? "chatgpt",
    ...(account.authIdentity || account.email
      ? { authIdentity: account.authIdentity ?? account.email }
      : {}),
  };
}
...
export function projectCodexAccountAuth(
  ...
  const accounts = snapshot.accounts.map(withChatgptAuth);
  if (!homeAuth || homeAuth.kind !== "api") {
    return { ...snapshot, accounts };
  }
  const identity = homeAuth.identity;
  if (accounts.length === 0 || snapshot.currentAccountId === null) {
    ...
    return {
      ...
      accounts: [
        apiAccount,
        ...accounts.filter((account) => account.accountId !== apiAccount.accountId),
      ],
    };
  }
  return {
