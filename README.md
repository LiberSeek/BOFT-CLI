...
>
> 表中的 — 表示 Host 尚未接入对应能力，不代表 CLI 本身没有。图片输入和 Teams 专用界面也未接入。版本、远程运行条件和验证范围见 [CodeBuddy 接入说明](docs/codebuddy-harness-integration.md)。Cursor CLI 仍为实验性接入，见 [Cursor CLI 实验说明](docs/cursor-cli-experimental.md)。
